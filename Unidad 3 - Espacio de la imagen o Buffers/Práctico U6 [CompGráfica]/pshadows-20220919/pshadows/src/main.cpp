...
int main() {
	...
	do {
		...
		glEnable(GL_STENCIL_TEST);// se habilita el test stencil
		
		glDepthFunc(GL_NEVER); // test-z siempre falla para que nada se dibuje en pantalla.
		glStencilFunc(GL_NEVER,1,~0); // el test de stencil falla siempre (cuando falle se ponga en 1)
		
		glStencilOp(GL_REPLACE,GL_KEEP,GL_KEEP); //reemplaza con 1 cuando falla
		drawFloor(true); //dibuja el piso iluminado
		
		glStencilFunc(GL_NOTEQUAL,1,~0); //falla cuando es 1 (pasa solo la mesa y no la sombra)
		glStencilOp(GL_INCR,GL_KEEP,GL_KEEP); //se incrementa el valor donde falla
		drawObject(shadow); // dibuja la sombra donde es 1
		glDepthFunc(GL_LESS); //se setea el test-z en menor
		
		/*glDisable(GL_STENCIL_TEST);*/
		// draw objects
		/// @todo: seleccionar la mascara y el valor de referencia adecuado para cada objeto
		glStencilFunc(GL_LEQUAL,1,~0); // pasa cuando es mayor o igual a 1 (piso y sombra)
		glStencilOp(GL_KEEP,GL_KEEP,GL_KEEP); // no se actualiza el stencil
		drawObject(reflection);// se dibuja la refelxion cuando el stencil es 1 y 2
		
		glStencilFunc(GL_EQUAL,2,~0); // pasa cuando es igual a 2 (sombra) 
		drawFloor(false);  //piso sin iluminar (la sombra)
//		drawObject(shadow);
		
		glStencilFunc(GL_EQUAL,1,~0);  // pasa cuando es igual a 1 (piso) 
		drawFloor(true); 
		glDisable(GL_STENCIL_TEST);//deshabilito el stencil test

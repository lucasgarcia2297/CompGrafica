...
//	float c;
	for(size_t i=0;i<initNodoSize;i++) { 
		glm::vec3 f(0.f,0.f,0.f); ///Acumulador para promedio de centroides de elementos originales
		glm::vec3 r(0.f,0.f,0.f); ///Acumulador para promedio de aristas
		glm::vec3 rFrontera(0.f,0.f,0.f); 
		glm::vec3 p = mesh.n[i].p; ///posicion original
		int n = 0; ///nro de elementos que conecta el nodo.
		
		///Recorro todos los elementos de la malla
		for(int ie : mesh.n[i].e){
//		for(size_t j=0;j<mesh.e.size();j++) { 
//			///Para cada elemento recorro todos sus vértices (nodos)
//			for(int k=0;k<mesh.e[j].nv;k++) { 
//				///Si llego a un vértice que pertenece a un nodo inicial, entro en el if 
//				if(mesh.e[j][k]==i){
					glm::vec3 prom(0.f,0.f,0.f);
					///Si la arista es frontera, 
					r += mesh.n[mesh.e[ie][1]].p;
					f += mesh.n[mesh.e[ie][0]].p;
					
					if(mesh.n[mesh.e[ie][1]].es_frontera){
						rFrontera += mesh.n[mesh.e[ie][1]].p;
					}
					if(mesh.n[mesh.e[ie][3]].es_frontera){
						rFrontera += mesh.n[mesh.e[ie][3]].p;
					}
...
//		}
		...
		///Recalculo la posicion del nodo
		if(!mesh.n[i].es_frontera) mesh.n[i].p = ((r*4.f)-f+p*((n-3)*1.f))/(n*1.f); ///nodos del borde
		else mesh.n[i].p = ((rFrontera/2.f)+p)/2.f; ///nodos interiores
		
	}
//	for(int i =0; i<n_size; i++){ // Por cada Nodo n original
//		r = {0.f,0.f,0.f};
//		float c = 0.f;
